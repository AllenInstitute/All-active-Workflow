...
class Test_Model(TestCase):
    ...
    def test_Stage0_parameters(self):
        ...
        mouse_spiny_stage0_params = os.path.join(
            self.mouse_spiny_path, 'Stage0_parameters.json')
        mouse_spiny_stage0_mechs = os.path.join(
            self.mouse_spiny_path, 'Stage0_mechanism.json')
        ...
        mouse_aspiny_stage0_params = os.path.join(
            self.mouse_aspiny_path, 'Stage0_parameters.json')
        mouse_aspiny_stage0_mechs = os.path.join(
            self.mouse_aspiny_path, 'Stage0_mechanism.json')
    ...
    def test_Stage0_features(self):
        ...
        nwb_handler_spiny = NwbExtractor(
            cell_id_spiny, nwb_search_pattern=cell_id_spiny)
        ...
        mouse_spiny_stage0_features = os.path.join(
            self.mouse_spiny_path, 'Stage0_features.json')
        mouse_spiny_stage0_protocols = os.path.join(
            self.mouse_spiny_path, 'Stage0_protocols.json')
        ...
        self.assertAlmostEqual(
            model_features_spiny_true_dict, train_features_dict_spiny)
        self.assertAlmostEqual(
            model_protocols_spiny_true_dict, train_protocols_dict_spiny)

        # Mouse aspiny
        cell_id_aspiny = self.mouse_aspiny_id

        nwb_handler_aspiny = NwbExtractor(
            cell_id_aspiny, nwb_search_pattern=cell_id_aspiny)
        acceptable_stimtypes = ['Long Square']
        ...
        mouse_aspiny_stage0_features = os.path.join(
            self.mouse_aspiny_path, 'Stage0_features.json')
        mouse_aspiny_stage0_protocols = os.path.join(
            self.mouse_aspiny_path, 'Stage0_protocols.json')
        ...
        self.assertAlmostEqual(
            model_features_aspiny_true_dict, train_features_dict_aspiny)
        self.assertAlmostEqual(
            model_protocols_aspiny_true_dict, train_protocols_dict_aspiny)
    ...
    def test_Stage1_parameters(self):
        ...
        model_mechs_spiny, model_mechs_release = model_params_handler_spiny.\
            get_opt_mechanism(model_params_spiny,
                              model_params_release, param_bounds_path)
        model_params_dict_spiny = convert_model_params_to_dict(
            model_params_spiny)
        mouse_spiny_stage1_params = os.path.join(
            self.mouse_spiny_path, 'Stage1_parameters.json')
        mouse_spiny_stage1_mechs = os.path.join(
            self.mouse_spiny_path, 'Stage1_mechanism.json')
        ...
        model_params_dict_aspiny = convert_model_params_to_dict(
            model_params_aspiny)
        mouse_aspiny_stage1_params = os.path.join(
            self.mouse_aspiny_path, 'Stage1_parameters.json')
        mouse_aspiny_stage1_mechs = os.path.join(
            self.mouse_aspiny_path, 'Stage1_mechanism.json')
        model_params_aspiny_true = utility.load_json(
            mouse_aspiny_stage1_params)
    ...
    def test_Stage2_parameters(self):
        ...
        model_params_dict_spiny = convert_model_params_to_dict(
            model_params_spiny)
        mouse_spiny_stage2_params = os.path.join(
            self.mouse_spiny_path, 'Stage2_parameters.json')
        mouse_spiny_stage2_mechs = os.path.join(
            self.mouse_spiny_path, 'Stage2_mechanism.json')
        ...
        for mech_key, mech_val in model_mechs_spiny.items():
            self.assertEqual(
                set(model_mechs_spiny_true[mech_key]), set(mech_val))
        ...
        param_bounds_path = os.path.join(
            self.mouse_aspiny_path, param_bounds_file)
        param_rule_func = adjust_param_bounds
        model_params_aspiny, model_params_release = model_params_handler_aspiny.\
            get_opt_params(param_bounds_path, param_rule_func)
        model_mechs_aspiny, model_mechs_release = model_params_handler_aspiny.\
        ...
        model_params_dict_aspiny = convert_model_params_to_dict(
            model_params_aspiny)
        mouse_aspiny_stage2_params = os.path.join(
            self.mouse_aspiny_path, 'Stage2_parameters.json')
        mouse_aspiny_stage2_mechs = os.path.join(
            self.mouse_aspiny_path, 'Stage2_mechanism.json')
        model_params_aspiny_true = utility.load_json(
