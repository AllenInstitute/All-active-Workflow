...
class CellDataTool(object):
    ...
    def save_morph_data(self):
        ...
        if not os.path.exists(morph_stats_filename):
            ...
            swc_filename = [
                swc_file_ for swc_file_ in swc_filename if 'cell_types' in swc_file_][0]
...
# TODO: move premium queue + machine settings
    def save_cell_metadata(self, from_lims=False, premium_queue=False, **props):
        ...
        if os.path.exists(metadata_filename):
            ...
            if from_lims:
                lr = lims.LimsReader()
                self.nwb_path = lr.get_nwb_path_from_lims(cell_id)
                self.swc_path = lr.get_swc_path_from_lims(cell_id)
            ...
            if not self.nwb_path and not 'nwb_path' in props:
                ...
                src_nwb = props['nwb_path'] or self.nwb_path
                dest_nwb = 'cell_types/'
                shutil.copy(src_nwb, dest_nwb)
                # TODO: clarify the set_cell_metadata logic
...
def host_machine_props(premium_queue):
    cell_metadata = {}
    machine_name = socket.gethostname()
    cell_metadata['Machine'] = 'aws' if machine_name == 'master' else machine_name
    if 'cori' in cell_metadata['Machine'] and premium_queue:
