...
class Morph_handler(object):
    def __init__(self, morph_file,**kwargs):
    ...
    def calc_euler_angle(self,morph_data,morph_apical = None):
        ...
        r = R.from_rotvec(theta*axis_of_rot)
    ...
    def drawSphere(self,center_tuple):
        rad = self.soma_rad
    ...
    def draw_morphology(self,theta,axis_of_rot,reject_axon=True,**kwargs):
        ...
        for n in self.morph.compartment_list:
            ...
            for c in self.morph.children_of(n):        
                cx,cy,cz = self.shift_origin(np.array([c['x'],c['y'],c['z']]))
        ...
        if kwargs.pop('draw_sphere',None):
            (xs,ys,zs) = self.drawSphere(shifted_soma_coord)
