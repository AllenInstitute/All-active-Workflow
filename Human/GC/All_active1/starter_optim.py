        ...
        for sect in section_map.values():
            if sect != 'all':
                for rev in rev_potential:
                    ...
                    if rev == 'ena' and sect not in ['apical', 'basal']: # Na mechanism only in soma and axon
                        iter_dict =  {'param_name':rev, 'sectionlist':sect,
                                  'dist_type': 'uniform', 'value': 53,'type':'section'}
                    elif rev == 'ek':
                        iter_dict =  {'param_name':rev, 'sectionlist':sect,
                                  'dist_type': 'uniform', 'value': -107.0 ,'type':'section'}
                    model_params.append(iter_dict) 
