...
def get_optimizer(opt_config,select_stim_key):
    ...
    return opt,protocol_dict

def get_GA_individuals(checkpoint,off_spring_size):
    ...
    return min_fitness_individual_sorted, min_fitness_pop,gen_size



def get_GA_responses(opt,GA_responses_file,select_stim_key,individual_list):
    ...
    return selected_individuals_responses,param_names
    
def absolute_value_parameters(individual_list):
    abs_individual_list = []
    for individual in individual_list:
        abs_individual_list.append(list(map(abs,individual)))
    return abs_individual_list   




def create_anim_views(anim_prefix,index,select_stim_key,\
              ephys_data_select_stim,protocol_dict,param_names,
              abs_individual_list,selected_individuals_responses,
              min_fitness_pop):
    
    fname = '%s%03d.jpeg'%(anim_prefix,index)
    fig,ax = plt.subplots(1,3,figsize=(12,4),gridspec_kw = \
                                  {'width_ratios':[9, .8, 8]})
#    plt.subplots_adjust(left=0.05, right=0.98)
    ax[0].scatter(np.arange(len(param_names)), abs_individual_list[index], marker = 'o',
...
#    ax[1].set_yticks([])
    bar_width = 0.2
    opacity = 0.5
    ax[1].bar([0], [min_fitness_pop[index]], bar_width, alpha=opacity,\
              color='r')
    
    resp_time = selected_individuals_responses[index][select_stim_key+'.soma.v']['time']
    resp_voltage = selected_individuals_responses[index][select_stim_key+'.soma.v']['voltage']
...
def main():
    ...
    opt,protocol_dict = get_optimizer(opt_config)
    
    
    checkpoint_file = 'checkpoints/seed1.pkl'
    checkpoint = utility.load_pickle(checkpoint_file)
    off_spring_size = 512
    min_fitness_individual_sorted, min_fitness_pop,gen_size = \
                        get_GA_individuals(checkpoint,off_spring_size)
    abs_individual_list= absolute_value_parameters(min_fitness_individual_sorted)
    
    select_stim_key ='LongDC_46'
    GA_responses_file = 'GA_responses_stage0.pkl'
    selected_individuals_responses,param_names = \
                get_GA_responses(opt,GA_responses_file,select_stim_key,\
                     min_fitness_individual_sorted)
    ...
    for index in range(gen_size):
        file_path = create_anim_views(anim_prefix,index,select_stim_key,\
              ephys_data_select_stim,protocol_dict,param_names,
              abs_individual_list,selected_individuals_responses,
              min_fitness_pop)
